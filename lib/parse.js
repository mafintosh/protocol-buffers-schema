...
import { PACKABLE_TYPES, Schema, TokenCount, onSyntaxVersion, onPackageName, onEnum, onMessage, onOption, onImport, onExtend, onService } from "./parser-internals";
export function parse(from) {
    const tc = new TokenCount(Object.freeze(tokenise(from.toString())));
    const schema = new Schema;
    const call = v => v(schema, tc);
    while (!tc.done)
        switch (tc.peek()) {
            case 'syntax':
                if (tc.t !== 0)
    ...
    for (const ext of schema.extends)
        for (const msg of schema.messages) {
            if (msg.name === ext.name)
                for (const { fields } of ext.messages)
        ...
        for (const field of msg.fields) {
            if (field.packed && !PACKABLE_TYPES.includes(field.type)) {
                ...
                if (field.type.includes('.')) {
                    ...
                    let last = types.shift(), curr = schema.messages.find(v => v.name === last);
                    for (const type of types) {
                        if (curr)
                            curr = curr.messages.find(v => v.name === type);
                        if (curr)
                            last = type;
                    }
                    if (!curr)
